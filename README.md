...
Чтобы создать пользователя и сразу добавить его в группу, исользовал команду ```sudo useradd -m -G adm username``` 
...
Проверил, что пользователь создался и добавился ```cat /etc/passw``` 
...
Вывел имена сетевых интерфейсов с помощью команды ```ls```
...
![Alt text](src/screenshots/ip_.png)
...
Определил и отобразил внешний IP-адрес шлюза (ip) и внутренний IP-адрес шлюза, также известный как IP-адрес по умолчанию (gw). 
...
Установил статические (заданные вручную, а не полученные от DHCP-сервера) настройки IP, GW, DNS (используя публичные DNS-серверы, например, 1.1.1.1 или 8.8.8.8).

Для этого через редактор ```nano``` открыл и изменил .yaml файл ```sudo nano /etc/netplan/00-installer-config.yaml```

![Alt text](<src/screenshots/set ip 1.1.1.8.8.8.8.png>)

Сохранил изменения с помощью ```apply```

Перезагрузил виртуальную машину с помощью команды ```sudo reboot``` и убедился, что статические сетевые настройки (ip, gw, dns) соответсвуют заданным. 

Проверил это, выполнив команду ```ping ya.ru``` или ```ping 1.1.1.1```

![Alt text](<src/screenshots/Screenshot 2024-11-29 at 5.46.16 PM.png>)

![Alt text](<src/screenshots/ping 1111 8888.png>)

# Part 4. OS Update

Обновил системные пакеты до последней версии с помощью команды ```sudo apt upadate && sudo apt upgrade```![Alt text](<src/screenshots/Screenshot 2024-12-01 at 4.14.45 PM.png>)

![Alt text](<src/screenshots/Screenshot 2024-12-01 at 4.17.51 PM.png>)

Убедился, что пакеты обновились, снова прописав эту команду 
...
Разрешил пользователю, которого создал в Part 2, использовать ```sudo```. Для разрешения пользователю fungusgr использовать команду "sudo" добавил его в файл конфигурации sudoers командой ```sudo adduser fungusgr sudo```

Зашел на пользователя и изменил имя машины. После проверил внесенные изменения
...
Установил текстовый редактор JOE. ```sudo apt install joe```. VIM и nano уже встроены в систему
...
После установки, включил автоматический запуск службы SSH при каждой загрузке системы. Для этого выполнил команду ```sudo systemctl enable ssh```

![Alt text](src/screenshots/zapyskprikajdomzapyske.png)

Сбросил службу SSHd на порт 2022 с помощью ```sudo nano /etc/ssh/ssh_config``` и изменил Port 22 на Port 2022. Сохранил изменения и вышел 

![Alt text](src/screenshots/port2022.png)

Перезапустил службу SSH для применения изменений следующей командой ```sudo systemctl restart ssh```
...
Установил top и htop  ```sudo apt install htop```

![Alt text](src/screenshots/installhtop.png)

Запустил top соответствующей командой 

![Alt text](src/screenshots/top.png)

После запуска команды top,  получил следующую информацию:
...
Использовал команду du с ключами -b для вывода размера в байтах и -s для суммарного размера:

```sudo du -sb /home /var && sudo du -s /var/log```

![Alt text](src/screenshots/dusb.png)

Чтобы вывести размер каждого элемента использовал команду ```sudo du -h * /var/log```
...
Установил утилиту ncdu (NCurses Disk Usage)

![Alt text](src/screenshots/ncduinstall.png)

Вывел размер home ```sudo ncdu /home```
...
Открыл для просмотра, с помощью команды ```less```
...
Для перезапуска службы SSHd выполнил следующую команду ```sudo systemctl restart sshd``` и  ```journalctl -t sshd```
посмотрел сообщение о перезапуске службы 
...
Используя планировщик заданий, запустил команду uptime каждые 2 минуты. Для этого испрльзовал команду ```crontab -e``` для редактирования и добавим строчку ```*/2 * * * * uptime```
